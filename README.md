...
Each entry includes reproduction commands, the raw trace, an interpretation, caveats, and a disclosure recommendation. **Two CVE-track classes are now in flight: SSRF in stdio fetch servers (2 packages, disclosed; 1 vendor-declined with a branch-verified fix left unmerged, 1 maintainer-confirmed unmaintained) and DNS rebinding in HTTP-transport servers (4 packages, all disclosed).**
